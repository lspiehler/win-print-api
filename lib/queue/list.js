...
var getPrinterData = function(params, callback) {
    if(config.USEFILES) {
        fs.readFile('./exports/printers.json', 'utf8', function(err, data) {
            if(err) {
                callback(err, false);
            } else {
                callback(err, data);
        ...
        powershell.runCommand({ cmd: cmd }, function(err, printresp) {
            if(err) {
                ...
                callback(false, printresp.stdout.toString());
            }
        });
    }
}
...
var getPrinters = function(params, callback) {
    getPrinterData(params, function(err, printresp) {
        if(err) {
            callback(err, false);
        } else {
            let ports = JSON.parse(printresp);
            let printers = {};
            let usewmi = false;
            if(config.USEWMI) {
                usewmi = true;
            ...
            for(let i = 0; i <= ports.length - 1; i++) {
                ...
                if(config.USEWMI) {
                    printers[ports[i].Name]['JobCount'] = 0;
                }
            }
