...
var finalizeRequest = function(ports, printers) {
    ...
    for(let i = 0 ; i <= keys.length - 1; i++) {
        if(ports.hasOwnProperty(printers[keys[i]].PortName)) {
            if(ports[printers[keys[i]].PortName].hasOwnProperty('PrinterHostAddress')) {
                printers[keys[i]].PrinterHostAddress = ports[printers[keys[i]].PortName]['PrinterHostAddress'];
            } else {
...
var getPrinters = function(params, callback) {
    powershell.runCommand({ cmd: 'Get-Printer | ConvertTo-Json' }, function(err, printresp) {
        if(err) {
            callback(err, false);
        } else {
